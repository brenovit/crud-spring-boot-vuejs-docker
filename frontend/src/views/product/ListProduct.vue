<template>
  <div>
    <div class="row mb-2">
      <router-link :to="{name: 'product-add'}" class="btn btn-success col-sm-2">Add product</router-link>
    </div>
        ...
        <table class="table table-sm table-hover">
          ...
          <tbody>
            <tr v-for="(product,index) in products" :key="product.id">
              ...
              <td>
                <router-link
                  :to="{name: 'product-view', params: {product_id: product.id}}"
...
export default {
  ...
  watch: {
    searchKey(newQuestion, oldQuestion) {
      productsSearch = this.productsFixed.filter(
        product =>
  ...
  methods: {
    ...
    deleteProduct() {
      ...
      api.findAll(r => {
        (this.products = r.data), (this.productsFixed = r.data);
      });
    }
  },
  mounted() {
